    ...
    <div className="max-w-3xl">
      ...
      {!currentVersion ? (
        ...
        <Panel tone="warn">
          ...
          <p className="mt-1 text-xs text-[#888]">
            Couldn&apos;t reach the GitHub Releases API. The check runs when you
            open this page and is cached for an hour — try again shortly.
          </p>
        </Panel>
      ) : data.isUpToDate ? (
        <Panel tone="ok">
...
docker compose up -d`}
      ...
      <div className="mt-6 text-xs text-[#666]">
        The latest-release check is cached for an hour and refreshed on demand.
        Last check: {data?.cachedAt ? new Date(data.cachedAt).toLocaleString() : "never"}.
      </div>
    </div>
  );
}
